...
export function ArticleLayout({ lang, children }: { lang?: 'pt' | 'en'; children: React.ReactNode }) {
  useEffect(() => {
...
export function ArticleHeader({
  ...
  dateISO,
  readingTime,
  authorName = 'Paulo Victor Silva',
  authorUrl,
  authorBio,
  avatarSrc = '/foto-avatar-sm.webp',
  lang,
}: ArticleHeaderProps) {
  const resolvedAuthorUrl = authorUrl ?? (lang === 'pt' ? '/sobre-mim' : '/about')
  return (
...
export function ArticleFooter({ lang, utmCampaign }: ArticleFooterProps) {
  ...
  const fellowUrl = `https://maven.com/marily-nika/ai-pm-bootcamp?utm_source=psilva&utm_medium=casestudy&utm_campaign=${utmCampaign}`
  return (
    <footer className="mt-16 pt-8 border-t border-border">
      <div className="flex items-start gap-3 mb-6">
        ...
        />
        <div>
          <p className="font-medium text-foreground">Paulo Victor Silva</p>
      ...
      <Link
        to={lang === 'pt' ? '/sobre-mim' : '/about'}
        className="inline-block text-sm text-primary hover:underline transition-colors mb-6"
      >
        {lang === 'pt' ? 'Más sobre el autor →' : 'More about the author →'}
      </Link>
      <div className="flex gap-3 mb-8">
        <a href="https://linkedin.com/in/paulovi" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-[#0A66C2]/10 border border-[#0A66C2]/20 text-sm font-medium text-[#0A66C2] hover:bg-[#0A66C2]/20 transition-colors">
        ...
        <a href="https://github.com/paulosilva" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-card border border-border text-sm font-medium text-muted-foreground hover:text-foreground hover:border-primary/30 transition-colors">
      ...
      <div className="flex items-center gap-3 text-xs text-muted-foreground">
        <span>&copy; {new Date().getFullYear()} Paulo Victor Silva. {f.copyright}</span>
        <span className="text-border">|</span>
        <Link to={lang === 'pt' ? '/privacidade' : '/privacy'} className="hover:text-primary transition-colors">
